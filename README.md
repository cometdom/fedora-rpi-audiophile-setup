...
- [x] **`v1.0.0`** — first tagged release of the ARM line. Runs end-to-end on a Raspberry Pi 5 (Fedora 44 ARM64): preflight, RT kernel, system-tuning, network, CPU states, DRUP + slim2Diretta with their web UIs. Validated on real hardware by early testers (Auke, ditusade, Progman). _A from-scratch fresh-image run and slim2Diretta-on-ARM are still welcome confirmations._
